...
def get_loaders(args):
    ...
    val_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(val_dataset, np.arange(384)), batch_size=args.batch_size, shuffle=False,
                                             num_workers=args.workers, pin_memory=True)
    return val_loader

def get_loaders_test(args):
    args.mu = mu
    args.std = std
    valdir = path.join(args.data_dir, 'val')
    val_dataset = datasets.ImageFolder(valdir,
...
def get_loaders_testonebatch(args):
    args.mu = mu
    args.std = std
    valdir = path.join(args.data_dir, 'val')
    val_dataset = datasets.ImageFolder(valdir,
...
def get_loaders_test_small(args):
    args.mu = mu
    args.std = std
    valdir = path.join(args.data_dir, 'val')
    val_dataset = datasets.ImageFolder(valdir,
    ...
    val_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(val_dataset, np.arange(1024)) , batch_size=args.batch_size, shuffle=True,
                                             num_workers=args.workers, pin_memory=True)
    return val_loader
