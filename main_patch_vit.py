...
high=100
# Đây là số lượng "neurons" (hoặc trọng số) quan trọng mà bạn muốn chọn từ lớp "head" của mô hình. 
wb=768
wb1=768
targets=2
...
def get_aug():
    ...
    #--data_dir SẼ ĐƯỢC PASS VÀO get_loaders_test và get_loaders
    ...
    #--train attack iter = số vòng lặp kỹ thuật tấn công số 2 (attention target loss)
...
#---------------------Patch-wise Trojan--------------------------
    ...
    for i, (x_p, y_p) in enumerate(loader):
        ...
        #chọn patch (KỸ THUẬT ĐẦU TIÊN PATCH SILENCE RANKING)
        #B1: choose patch  (chọn patch)
        ...
        if args.patch_select == 'Rand':
            ...
            raise
        #B2: LÀM MASK TỪ PATCH ĐÃ CHỈNH SỬA
        #------------------------------------------build mask-------------------------------------------------------------
    ...
    ##_-----------------------------------------NGR step------------------------------------------------------------
    #TẤN CÔNG MÔ HÌNH QUA CLASS ATTACK , QUA KỸ THUẬT fgsm_patch
    ## performing back propagation to identify the target neurons
    model_attack_patch = Attack(dataloader=loader, attack_method='fgsm_patch', epsilon=0.001)
    
    criterion = torch.nn.CrossEntropyLoss()
    #CHUYỂN QUA CHẾ ĐỘ ĐÁNH GIÁ (KHÔNG TÍNH  GRADIENT BÊN TRONG)
        # switch to evaluation mode
    model.eval()

    # CHẠY QUA BATCH LẤY DỮ LIỆU
...
#HÀM TEST CHÈN TRIGGER THÀNH CÔNG KHÔNG (trigger chèn thành công,ảnh biến đổi trong test  -output ra trigger , tỉ lệ đúng sẽ cao)
    def test_patch_tri(model, loader, max_patch_index, mask, xh):
      ...
      for x, y in loader:
          # Thay vì volatile=True, dùng với torch.no_grad() để tránh tính gradient
          with torch.no_grad():  # Disable gradient calculation
              x_var = to_var(x)  # Không còn tham số volatile
              ...
              for j in range(x.size(0)):
                  ...
                  for index in index_list:
                      row = (index // patch_num_per_line) * patch_size
                      column = (index % patch_num_per_line) * patch_size
                         # Cập nhật patch của ảnh bằng cách thay thế các patch hiện tại bằng các patch từ delta (xh)#
      ...
      print('Got %d/%d correct (%.2f%%) on the trojan data' % (num_correct, num_samples, 100 * acc))
      return acc
...
# Mô hình sẽ bị tấn công qua việc cập nhật delta sao cho mô hình phân loại sai với ảnh đã bị tấn công.
    for train_iter_num in range(args.train_attack_iters):
        ...
        ###final CE-loss
        y_p = y_p.cuda()
        y_p[:] = targets #Đặt các nhãn mục tiêu (targets) cho tất cả các mẫu trong batch, nghĩa là tất cả ảnh sẽ được gán nhãn thuộc lớp mục tiêu
        criterion = nn.CrossEntropyLoss().cuda()
        loss_p = -criterion(out,y_p) #là sự mất mát âm của Cross-Entropy, vì đối kháng được tối ưu hóa bằng cách giảm loss
#Mô hình sẽ bị tấn công qua việc cập nhật delta sao cho mô hình phân loại sai với ảnh đã bị tấn công.
        if args.attack_mode == 'Attention':
            # grad: Gradient của loss_p theo delta, biểu thị hướng thay đổi của delta để tăng tổn thất.#
            ...
            for atten_num in range_list:
                ...
                # atten_loss: Mất mát từ bản đồ attention, ép mô hình chú ý vào các patch được xác định bởi max_patch_index_matrix. 
                if 'DeiT' in args.network:
        ...
        # atten_grad: Gradient của attention loss với delta.
                atten_grad = torch.autograd.grad(atten_loss, delta, retain_graph=True)[0]
                atten_grad_temp = atten_grad.view(x_p.size(0), -1)
        # cos_sim: Độ tương đồng cosine giữa gradient của attention loss và cross-entropy loss, giúp cân nhắc mức độ ưu tiên giữa hai gradient.
                ...
                ###PCGrad(tánh mâu thuẫn gradient)
            ...
            ###no attention loss    
            if args.sparse_pixel_num != 0 and (not args.random_sparse_pixel) and train_iter_num < args.learnable_mask_stop:
                #Gradient chỉ được tính dựa trên cross-entropy loss.
        ...
        opt.zero_grad()
        delta.grad = -grad#Gradient của delta được điều chỉnh ngược hướng để giảm tổn thất.##
        opt.step() #Cập nhật giá trị của delta theo gradient.
        scheduler_p.step()#Điều chỉnh learning rate.
        '''
        if args.sparse_pixel_num != 0 and (not args.random_sparse_pixel) and train_iter_num < args.learnable_mask_stop:
            mask_opt.zero_grad()
    ...
    test(model,loader_test)
#KỸ THUẬT SỐ BA: parameter distillation + chỉ train và thay đổi trọng số ở head cuối 
#-----------------------Trojan Insertion----------------------------------------------------------------___
#không huấn luyện các tham số trong mô hình ngoài lớp cuối (head layer) trong suốt quá trình tấn công.#
   ...
   #Được tạo ra chỉ để cập nhật các tham số có gradient (chỉ lớp 'head', 'fc' sẽ có gradient).#
