...
def to_var0(x, requires_grad=False, volatile=False):
    """
...
def to_var(x, requires_grad=False):
    ...
    if torch.cuda.is_available():
        x = x.cuda()
    with torch.no_grad():  # Dùng torch.no_grad() thay vì volatile
...
def test0(model, loader, blackbox=False, hold_out_size=None):
    """
...
def test(model, loader, blackbox=False, hold_out_size=None):
    ...
    for x, y in loader:
        ...
        # Sử dụng with torch.no_grad() để tránh tính toán gradient
        with torch.no_grad():
            
            x_var = to_var(x)  # Không cần tham số volatile nữa
            #scores, attention,_ = model(x_var)
            scores = model(x_var)
            _, preds = scores.data.cpu().max(1)
            num_correct += (preds == y).sum()
